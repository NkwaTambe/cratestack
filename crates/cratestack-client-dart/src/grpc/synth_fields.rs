...
/// (`crate::naming::scalar_model_fields`, minus generated-on-create), so
/// the gRPC wire shape matches the already-generated `models.dart` data
/// class field-for-field. `model_names` must be every model name in the
/// *schema*, not just this one — `scalar_model_fields`'s relation filter
/// checks `field.ty.name` against it, and a relation almost always points
/// at a *different* model name than the one being built.
pub(super) fn scalar_fields_for_create(model: &Model, model_names: &BTreeSet<&str>) -> Vec<Field> {
    crate::naming::scalar_model_fields(model, model_names)
        .into_iter()
        .filter(|field| !crate::naming::is_generated_on_create(field))
        .cloned()
        .collect()
}
...
pub(super) fn scalar_fields_for_update(model: &Model, model_names: &BTreeSet<&str>) -> Vec<Field> {
    crate::naming::scalar_model_fields(model, model_names)
        .into_iter()
        .filter(|field| !crate::naming::is_primary_key(field))
        .cloned()
        .collect()
}
...
/// them into `hasNextPage`/`hasPreviousPage` on the decoded-map side, to
/// match `models.dart.j2`'s hand-written `PageInfo` class.
